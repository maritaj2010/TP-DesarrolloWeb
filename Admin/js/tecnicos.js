...
createApp({
  data() {
     ...
     */
    return {
      tecnicos: [], // Almacena los productos obtenidos del backend
      // url:'http://localhost:5000/serviciostecnicos', // URL local
      url: "https://trevize.pythonanywhere.com/serviciostecnicos", // URL del backend donde se encuentran los productos
      error: false,
      cargando: true,
  ...
  methods: {
    ...
    grabar() {
      /* El método grabar se encarga de guardar los datos de un nuevo producto en el servidor. 
      Primero, se crea un objeto producto con los datos ingresados en el formulario. 
      Luego, se configuran las opciones para la solicitud fetch, incluyendo el cuerpo 
      de la solicitud como una cadena JSON, el método HTTP como POST y el encabezado Content-Type 
      como application/json. Después, se realiza la solicitud fetch a la URL especificada utilizando 
      las opciones establecidas. Si la operación se realiza con éxito, se muestra un mensaje de éxito 
      y se redirige al usuario a la página de productos. Si ocurre algún error, se muestra un mensaje 
      de error.
       */
      ...
      var options = {
        body: JSON.stringify(tecnico), // Convertir el objeto a una cadena JSON
      ...
      fetch(this.url, options)
        .then(function () {
          ...
          window.location.href = "./tecnicos.html"; // Redirigir a la página de productos
        })
        .catch((err) => {
          console.error(err);
